              ...
              "
                ...
                <div
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <!-- update-begin-author:hswift date:20210308 for：popuphswift -->
                    <template v-else-if="col.type === formTypes.popuphswift">
                      <a-tooltip
                        :key="i"
                        :id="id"
                        placement="top"
                      ...
                      >
                        ...
                        <span
                          ...
                          <j-select-material
                            :multi="false"
                            :value="getPopuphswiftValue(id)"
                            @change="(v)=>handleChangePopuphswiftCommon(v,id,row,col,rowIndex)" />
                        </span>
                      </a-tooltip>
                    </template>
                    <!-- update-end-author:hswift date:20210308 for：popuphswift -->
  ...
  export default {
    ...
    methods: {
      ...
      _pushByDataSource(dataSource, insertIndexes = null, update = true, rows = null, setDefaultValue = false) {
        ...
        let popuphswiftValues = { ...this.popuphswiftValues }
        ...
        dataSource.forEach((data, newValueIndex) => {
          ...
          this.columns.forEach(column => {
            ...
            } else if (column.type === FormTypes.popuphswift) {
              popuphswiftValues[inputId] = sourceValue
        ...
        this.popuphswiftValues = popuphswiftValues
        ...
        for (let value of inputValues) {
          ...
          this.columns.forEach(column => {
            ...
            if (column.type === FormTypes.checkbox) {
              ...
              if (!value[column.key]) {
                value[column.key] = this.popuphswiftValues[inputId] || null
              }
            } else if (column.type === FormTypes.radio) {
      ...
      _getAllValuesForSlot() {
        return cloneObject({
          ...
          popuphswiftValues: this.popuphswiftValues,
        ...
        values.forEach(item => {
          ...
          for (let newValueKey in newValues) {
            if (newValues.hasOwnProperty(newValueKey)) {
              ...
              // 在 popuphswiftValues 中寻找值
              if (!edited) {
                edited = this.setOneValue(this.popuphswiftValues, modelKey, newValue)
              }
              // 在 radioValues 中寻找值
              if (!edited) {
      ...
      handleChangePopuphswiftCommon(value, id, row, column,index) {
        this.popuphswiftValues = this.bindValuesChange(value, id, 'popuphswiftValues')
        // 做单个表单验证
        this.validateOneInput(value, row, column, this.notPassedIds, true, 'change')

        // 触发valueChange 事件
        this.elemValueChange(FormTypes.popuphswift, row, column, value)
      },
      ...
      /** popuphswift回调 */
      popuphswiftCallback(value, others, id, row, column, index) {
        // 存储输入的值
        this.popuphswiftValues[id] = value
        if (others) {
          Object.keys(others).map((key) => {
            this.columns.map(k=>{
              if(k.key === key){
                ...
                if(k.type === 'date'){
                  ...
                  this.popuphswiftValues[index][key] = others[key]
                }
              }
            })
          })
      ...
      /** popuphswift输入框回显 */
      getPopuphswiftValue(id) {
        return this.popuphswiftValues[id]
      },
